...
export const TruthOfTheDay = () => {
    ...
    const [month, monthRaw, day, year] = useMemo(() => {
        ...
        return [date.toLocaleString('default', { month: 'long' }), date.getMonth(),date.getDate(),date.getFullYear()]
    },[]);
    ...
    return <FlexBox orientation='row' style={{marginBottom:".5rem"}}>
        <SiteText bold value="Truth of the day" style={{marginRight:".5rem"}}/>
        {<div style={{flexGrow:1}}>{!isNil(truth.link) ? 
        <Anchor link={truth.link}>
            <SiteText value={truth.text}/>
            </Anchor>: <SiteText value={truth.text}/>}</div>}
